...
SAMPLE_COLOR = "#e60000"
SCENARIO_DIR = Path(__file__).resolve().parent.parent / "scenarios/10x6_labyrinth"
...
def dense_grid_field(
    ...
    return (
        ...
        np.ma.array(speed, mask=outside),
    )
...
def draw_estimator_connector(fig, axes) -> None:
    """Draw up-right-down connector between both subplot centers."""

    bbox_left = axes[0].get_position()
    bbox_right = axes[1].get_position()

    x_left = 0.5 * (bbox_left.x0 + bbox_left.x1)
    x_right = 0.5 * (bbox_right.x0 + bbox_right.x1)

    y_start = max(bbox_left.y1, bbox_right.y1)
    y_mid = y_start + 0.115

    connector_path = MplPath(
        [(x_left, y_start), (x_left, y_mid), (x_right, y_mid), (x_right, y_start)],
    ...
    box_width = 0.20
    box_height = 0.08
    box_x = 0.5 * (x_left + x_right) - 0.5 * box_width
    box_y = y_mid - 0.5 * box_height
...
def add_panel_labels(fig, axes) -> None:
    for ax, label in zip(axes, ("Sparse Measurements", "Airflow Field Estimate")):
...
def main() -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(3.45, 1.45))
    ...
    draw_estimator_connector(fig, axes)
